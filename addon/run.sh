...
# The user MUST edit this to add at least one circuit + battery (with a
# Shelly Plug PM Gen3) before the dispatcher does anything useful.
# real_shelly host/port are passed as CLI flags on every start so the HA
# Configuration tab always wins.
